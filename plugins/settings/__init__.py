...
class Settings(ScrollArea):
    ...
    def __init__(self, mainwindow: QMainWindow, path: str):
        super().__init__()
        ...
        self.cfg = AppConfig()
        qconfig.load(sltk.join_path(path, 'config.json'), self.cfg)
        self.scGroup_theme = SettingCardGroup('主题', self.container)
        self.sc_theme_style = ComboBoxSettingCard(
            self.cfg.theme_style, FluentIcon.PALETTE, '主题', '设置主题', ['跟随系统', '浅色', '深色'])
        self.sc_theme_color = CustomColorSettingCard(
            self.cfg.theme_color, FluentIcon.PALETTE, '主题颜色', '设置主题颜色')
        self.scGroup_theme.addSettingCards(
            [self.sc_theme_style, self.sc_theme_color])
        self.expendLayout.addWidget(self.scGroup_theme)

        self.scGroup_animation = SettingCardGroup('动画', self.container)
        self.sc_animation_switch = SwitchSettingCard(
            FluentIcon.ZOOM, '动画', '设置窗口切换动画开关', self.cfg.animation_switch)
        self.sc_animation_type = ComboBoxSettingCard(self.cfg.animation_type, FluentIcon.ZOOM, '动画', '设置动画样式', [
                                                     '线性插值', '四次插值', '三次插值', '正弦插值', '弹性插值', '回弹插值', '弹跳插值'])
        self.scGroup_animation.addSettingCards(
            [self.sc_animation_switch, self.sc_animation_type])
    ...
    def signal_connect(self):
        ...
        self.cmb_animation.currentTextChanged.connect(
...
class AppConfig(QConfig):
    ...
    theme_style = OptionsConfigItem('main', 'theme_mode', 'AUTO', OptionsValidator([
                                    'AUTO', 'LIGHT', 'DARK']), restart=True)
    theme_color = ColorConfigItem('main', 'theme_color', '#4cc2ff')
    animation_type = OptionsConfigItem('main', 'animation_type', 'InOutCubic', OptionsValidator(
        ['Linear', 'InOutQuad', 'InOutCubic', 'InOutSine', 'InOutElastic', 'InOutBack', 'InOutBounce']))
